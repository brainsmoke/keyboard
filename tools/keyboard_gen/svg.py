...
def circle(pos, radius, stroke="#000000", fill=None):
    if stroke is None:
        stroke='none'
    ...
    print( f'<circle cx="{pos[0]:f}" cy="{pos[1]:f}" r="{radius:f}" style="fill:{attr_escape(fill)};stroke:{attr_escape(stroke)};stroke-width:0.264583px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1" />' )
